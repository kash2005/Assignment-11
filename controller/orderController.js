...
itemIdCB.on("change", function() {
    ...
    let itemObj = $.grep(item_db, function(item) {
        return String(item.item_id) === selectedValue;
    });

    if (itemObj.length > 0) {
        // Access the first element in the filtered array
        itemName.val(itemObj[0].item_description); // Assuming there is an 'item_name' property
        price.val(itemObj[0].item_price);
        qtyOnHand.val(itemObj[0].item_qty);
    }

    // Check if the item is already in the items array
    let existingItem = items.find(item => item.item_id  === selectedValue);
...
customerIdCB.on("change", function() {
    ...
    let customerObj = $.grep(customer_db, function(customer) {
        console.log(String(customer.customer_id) === selectedValue);
        return String(customer.customer_id) === selectedValue;
    });
...
add.on("click", function () {
    ...
    if (Number(qtyOnHand.val()) >= qtyValue) {
...
submitBtn.on("click", function () {
    ...
    items.forEach(item => {
        ...
        item_db.forEach((itemObj) => {
            if (String(itemObj.item_id) === item.itemCode) {
                itemObj.item_qty -= item.qtyValue;
            }
        });
    });
